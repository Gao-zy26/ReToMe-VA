...
class AttentionControl_uc(abc.ABC):
    def __init__(self):
        self.cur_step = 0
        self.num_att_layers = -1
        self.cur_att_layer = 0
...
class AttentionStore_uc(AttentionControl_uc):
    def __init__(self, res):
        super(AttentionStore_uc, self).__init__()
        self.step_store = self.get_empty_store()
        self.attention_store = {}
        self.res = res
    ...
    def reset(self):
        super(AttentionStore_uc, self).reset()
        self.step_store = self.get_empty_store()
        self.attention_store = {}

class AttentionControlEdit_uc(AttentionControl_uc, abc.ABC):
    def __init__(self, num_steps: int,
                 self_replace_steps: Union[float, Tuple[float, float]], res, noFrames=16):
        super(AttentionControl_uc, self).__init__()
        self.batch_size = 2
        if type(self_replace_steps) is float:
            self_replace_steps = 0, self_replace_steps
        self.num_self_replace = int(num_steps * self_replace_steps[0]), int(num_steps * self_replace_steps[1])
